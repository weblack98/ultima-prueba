...
function createReel(reel) {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const clase = simbolos[Math.floor(Math.random() * simbolos.length)];
        symbolEl.className = "symbol " + clase;
...
spinBtn.addEventListener("click", async () => {
    ...
    for (let reel of reels) createReel(reel);

    for (let i = 0; i < reels.length; i++) {
        await spinReel(reels[i], 1500 + i * 500, i * 200);
    }

    let result = reels.map(r => r.children[2].className);
